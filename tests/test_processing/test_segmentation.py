...
SemLabels = seg_proc.SEMSegmentationLabels


@patch.object(seg_proc, "resize_image")
...
def test_get_gis_thickness_equivalent_to_old_method(scaling: int | None) -> None:
    ...
    gis_thickness_old = seg_proc.get_gis_thickness_old(
        prediction=prediction,
    ...
    gis_thickness_clean_edges, xlims_out = seg_proc.get_gis_thickness(
        prediction=prediction,
        xlims=xlims,
        ylims=(ylims[0], None),
        image_shape=image_shape,
...
def test_keep_only_largest_object(dtype: type, connectivity: int) -> None:
    ...
    output = seg_proc.keep_only_largest_object(
...
def test_get_lamella_centre() -> None:
    ...
    found_centre = seg_proc.get_lamella_centre(test_lamella.array)
...
def test_methods_equivalent_for_simple_rectangle() -> None:
    ...
    centre_2 = seg_proc.get_lamella_centre(
