...
def find_trough_peaks_from_thresholded_image(
    ...
    distance_px: int,
    width_px: float | tuple[float, float] = 1,
    ...
    if centre:
        # Use centre from edges rather than the actual peak
        peaks = np.asarray(
            [np.mean(_) for _ in zip(properties["right_ips"], properties["left_ips"])]
        )

    confidence = properties["prominences"] * properties["widths"]

    sorted_idxs = np.argsort(confidence)[::-1]  # most prominent -> least prominent
    peaks_mask = np.ones_like(peaks, dtype=np.bool_)

    # Group peaks to most prominent within distance_px
    for i in sorted_idxs:
        if not peaks_mask[i]:
            # ignore peaks that have already been filtered out
            continue
        peaks_mask &= np.abs(peaks - peaks[i]) > distance_px
        peaks_mask[i] = True
    ...
    sorted_prominence_valid_idxs = np.argsort(confidence[peaks_mask])[
        ::-1
    ]  # most confident -> least confident

    prominence_sorted_valid_peaks = peaks[peaks_mask][sorted_prominence_valid_idxs]
    if expected_count != -1:
        if prominence_sorted_valid_peaks.size < expected_count:
            raise ValueError("Not enough valid peaks found")
        prominence_sorted_valid_peaks = prominence_sorted_valid_peaks[:expected_count]
    return sorted(prominence_sorted_valid_peaks)  # sorted by peak value
...
def find_centre(
    ...
    try:
        ...
        large_padding = int(lamella_width_px * lamella_pad)

        sigma_large = 100
        sigma_medium = 20
        sigma_small = 4
        lamella_height_sigma = max(lamella_height_px / 12 - 2, 0)

        y_sigma = sigma_small + lamella_height_px / 100
        x_sigma = 10
        gaussian = ndi.gaussian_filter(data, (y_sigma, x_sigma))
        ...
        centre_x = find_trough_peaks_from_thresholded_image(
            util.img_as_uint(
                util.invert(ndi.gaussian_filter(data, (sigma_large, sigma_medium)))
            ),
            distance_px=lamella_width_px,
            width_px=(
                lamella_width_px * (1 - lamella_pad),
                lamella_width_px * (1 + lamella_pad),
            ),
            expected_count=1,
            centre=True,
        ...
        blurred_trench = ndi.gaussian_filter(
            trench, lamella_height_sigma, axes=0, mode="constant", cval=0
        )

        # blurred_trench[blurred_trench < gaussian_triangle_threshold] = 0
        # fig = filters.try_all_threshold(blurred_trench)[0]
        mean_trench = util.invert(blurred_trench).mean(axis=1).squeeze()
        threshold_crop = int(np.ceil(lamella_height_sigma))
        threshold_crop_idxs = (threshold_crop, mean_trench.size - threshold_crop)
        lamella_y_threshold = filters.threshold_triangle(
            mean_trench[
                threshold_crop_idxs[0] : threshold_crop_idxs[1]
            ]  # don't include edges
        )
        trench_line = mean_trench <= lamella_y_threshold

        idxs = np.argwhere(~trench_line).flatten()
        idxs = (
            max(idxs[0], threshold_crop_idxs[0]),
            min(idxs[-1], threshold_crop_idxs[1]),
        )
        indexes_1[0] = idxs

        mean_trench = util.invert(mean_trench)

        lamella_y_threshold = filters.threshold_triangle(mean_trench[idxs[0] : idxs[1]])
        trench_line = mean_trench[idxs[0] : idxs[1]] > lamella_y_threshold

        peaks, peak_properties = find_peaks(
            trench_line,
            distance=lamella_height_px / 2,
            width=(lamella_height_px * 0.25, lamella_height_px * 2),
        )
        ...
        if len(peaks) == 1:
            ...
            raise ValueError("Not peak found")

        lamella_peak = peaks[peak_idx] + idxs[0]
        lamella_edges_y = (
            peak_properties["left_ips"][peak_idx] + idxs[0],
            peak_properties["right_ips"][peak_idx] + idxs[0],
        )
        ...
        indexes_2 = [
            ...
            (
                ...
                min(
                    int(centre_x + lamella_width_px / 2 + large_padding), data.shape[1]
                ),
            ),
        ]

        sobel_thresholded2 = sobel_v[
        ...
        final2 = morphology.remove_small_objects(
            sobel_thresholded2, min_size=data.shape[0] // 3
        )

        distances_x2 = find_trough_peaks_from_thresholded_image(
            final2,
            distance_px=lamella_width_px * (1 - lamella_pad),
            width_px=1,
            expected_count=2,
        )
        ...
        if plot_path is not None:
            ...
            try:
                ...
                axs[1].axvline(
                    ...
                    linewidth=linewidth,
                    color="C1",
                    label="Detected centre of trough",
                )
            ...
            try:
                ...
                logging.error("Failed to save figure %s", plot_path, exc_info=True)
...
if __name__ == "__main__":
    ...
    centring_states = {_.name: 0 for _ in CentringStates}
    ...
    while True:
        try:
            ...
            centre_state: CentringStates = CentringStates.SUCCESS
            try:
                if plot_centres:
                ...
                try:
                    ...
                    centre_state += CentringStates.FAILED_SEM
                    logging.error(
                        "Failed %s", elecron_centre_plot_path.stem, exc_info=True
                    )
                try:
                    ...
                    centre_state += CentringStates.FAILED_FIB
                    logging.error("Failed %s", ion_centre_plot_path.stem, exc_info=True)
                # if plot_centres:
                ...
                centring_states[CentringStates(centre_state).name] += 1
