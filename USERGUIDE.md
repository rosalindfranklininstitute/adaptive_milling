...
The defaults are recommended as a good starting place for the settings that have them.
...
| **Model generation**                | *Latest* | The generation of model to use.                                                                        | Defaults to the latest model generation if not specified. All current models are "1.0".                                                                                                             |
| **Minimum GIS thickness threshold** | 0.25 µm  | Stop polishing if the local GIS thickness drops below this threshold.                                  | Consider adjusting this based on risk tolerance. Can be disabled by setting to 0.                                                                                                                   |
| **Median GIS thickness threshold**  | 0.30 µm  | Stop polishing if the median GIS thickness across the width of the lamella drops below this threshold. | Consider adjusting this based on risk tolerance. Can be disabled by setting to 0.                                                                                                                   |
| **Max. crack area**                 | 0.20µm²  | Stop polishing if the total crack area is above this threshold.                                        | Consider adjusting this based on your tolerance to cracks/holes.                                                                                                                                    |
| **Max. cycles**                     | 60       | Maximum adaptive polish cycles per lamella.                                                            | This should rarely (if ever) be why Adaptive Polishing stops, so can be set fairly high and forgotten.                                                                                              |

There are also advanced options that we *strongly* recommend leaving as their defaults but can be accessed by clicking the sliders/toggles button:

| Advanced Setting          | Default | Description                                                                           | Notes                                                                                                                                                                                         |
| ------------------------- | ------- | ------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Min. lamella area**     | 30 µm²  | Stop polishing if the segmented lamella area is smaller than this size.               | This may indicate that an alignment or major segmentation issue has occurred.                                                                                                                 |
| **Max. lamella drift**    | 70 µm   | Stop polishing if the segmented lamella area has moved more than this between cycles. | This may indicate that an alignment or major segmentation issue has occurred.                                                                                                                 |
| **Align SEM**             | True    | Centre the lamella in the SEM image before polishing.                                 | This corrects for any errors in coincident height when setting up the lamella site. This is highly recommended to be left on.                                                                 |
| **Save segmentations**    | True    | Saves the raw and clean SEM segmentations.                                            | These segmentations are useful for improving segmentation models.                                                                                                                             |
| **GIS filter sigma**      | 1.67    | Sigma of the Gaussian used to smooth the GIS thickness measurements.                  |                                                                                                                                                                                               |
| **X-padding fraction**    | 0.0     | The fraction of the lamella width that the X-limits will be padded by.                | These X-limits determine the region that GIS thickness will be measured.                                                                                                                      |
| **Imaging Field of View** | 40 µm   | The horizontal field width used for SEM imaging.                                      | This overrules the task acquisition parameters FOV. We recommend 40 µm, as we found that higher magnifications help model performance and most of the training data is at this magnification. |
